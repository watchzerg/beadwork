...
```bash
python3 <skill-dir>/scripts/beadwork.py verify phase --check-report preflight <report.json> --expected <dispatch.json> --emit-receipt
```

executor 缺可用 dispatch 时，可对按 schema 填写的部分报告做结构自检：

```bash
python3 <skill-dir>/scripts/beadwork.py verify ticket --check-report <report.json> --emit-receipt
```
...
```bash
python3 <skill-dir>/scripts/beadwork.py verify phase --check-report <preflight|finalizer> <report.json> --expected <dispatch.json> --emit-receipt
python3 <skill-dir>/scripts/beadwork.py verify worker --check-report <implementer|fixer|reviewer> <report.json> --expected <dispatch.json> --emit-receipt
```
