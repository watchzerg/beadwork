...
新派发采用 `workflow_contract_version: 4`。prior_finalization 指向原阶段 dispatch；同一 reviewed_main 恢复原 attempt 和检查点，保留 BASE、stage、fixer/review 的明确选择、累计 gates、commits、dirty 现场及 gate-fix 额度。先确认旧任务结束，再调用 final-stage continuation: resume；返回原 stage、selected_fixer、review_round、selected_review、selected_stage 和 context_sources。
