...
4. 完整 review 按 repair_route 分流；仅文档阻塞先完成 [文档收尾](document-closeout.md)，保留原 gate-full/review，不开启新的修复 stage。
5. 填写当前 draft_schema_path 的语义判断并组装：
...
代码候选需要同一干净 HEAD 的完整 gate-full、正常退出与运行来源绑定。纯文档收尾通过后，原 gate 继续绑定代码候选，文档交付 HEAD 由单独的限定验收覆盖。相同候选的有效结果复用；后续失败或无效验证使旧成功失效，修复后从完整入口重跑。未知运行填写实际收尾说明；来源损坏仅支持 blocked/interrupted 交付。
...
writer 的交付入口见 [writer-delivery.md](writer-delivery.md)。fixer DONE 后，finalizer 可补充验证，再进入 review；代码修复关联文档由 fixer 负责；review 后纯文档问题走限定收尾。补充验证在当前干净 HEAD 正常退出但发现代码失败时，组装 code_failure 并进入下一修复阶段，不重新打开已完成的 fixer；环境失败或未知运行仍按 blocked/interrupted 处理。

## 推进与 root 交付

- READY_TO_MERGE：文档覆盖完成，代码候选的完整 gate-full、双轴 review 与适用的文档收尾验收共同覆盖交付 HEAD；现场干净、任务结束、无 blockers/remaining_work。
