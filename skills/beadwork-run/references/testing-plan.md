...
- **Verification:** <具体测试命令或运行场景及预期结果，说明如何观察本票行为>
```
...
两种模式都必须声明 `Verification`，说明本票怎样证明 acceptance。涉及真实边界时，明确相应测试或场景；仅基础检查通过不能替代行为证据。测试无法收窄时声明完整相关 suite。选择入口与覆盖范围时，读取 `testing-gates.md`。

执行时基线已满足 Expected red 不属于计划结构错误：beadwork-run 按 `baseline-adaptation.md` 核准实际执行计划，保留原声明和 seam 授权。部分行为已有时，仅对剩余行为取得 red。

test mode 缺失或冲突、TDD 缺少 observable behavior / expected red 或 seam reference 无效、direct verification 缺少 reason、任一模式缺少 verification 或验证无法覆盖 acceptance 的 ticket，不得进入实现。此限制适用于 `ready-for-agent` 和无人值守执行；人工直接调用 `tdd` 且没有 assigned ticket 时保留交互确认。
