...
ticket 和新版 finalizer 的 collect 都会将当前 collection 绑定到 stage 检查点；阶段报告必须保留该选择。同 round 更正、组装前中断和旧阶段冻结的规则见 `ticket-execution.md`。finalizer 的更正同样清除旧阶段选择，须重新组装后才能交付或推进。
