...
- code_failure：executor 已用尽 stage 0..5，设为 blocked 并记录停止，不再自动派第七阶段。
...
若阻塞原因是 writer 在 `begin-gate-repair` 前已经提交修正，且旧阶段已有 repair 候选、review 未开始、当前现场是报告绑定的干净后继，解除阻塞后由原 executor 对 root 调用 `ticket-stage`，传入 `{"continuation":"recover","recovery_reason":"<具体原因>"}`。脚本追加恢复证据并进入下一 stage；不改写旧候选，不恢复旧 writer。其他 blocked 原因继续按原 stage 恢复。
