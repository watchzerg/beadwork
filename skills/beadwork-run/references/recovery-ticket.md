...
恢复前核实旧 executor、implementer、reviewers 及命令结束。未确认停止时不派接替 writer。具体恢复位置和禁止重置的计数见 ticket-execution.md；代码失败在 executor 内推进 stage，controller 不新增四阶段额度。已完成 ticket 应复用整票交付并关闭，不再恢复实现。
