...
def preflight_input(d):
    ...
    repository.require(accepted.get('kind') == 'mechanical_acceptance' and accepted.get('role') == 'preflight'
...
def contexts(d):
    ...
    for number, path in enumerate(sorted(root.glob('context-add-*.json')), 1):
        ...
        repository.require(path.name == f'context-add-{number:06d}.json' and value['previous'] == previous, '上下文补充链不连续')
...
def add_context(dispatch_path, facts):
    ...
    repository.require(set(facts) == {'sources', 'reason'} and isinstance(facts['reason'], str) and facts['reason'].strip(),
...
def close(dispatch_path, report_path, facts):
    ...
    repository.require(set(facts) == required and type(facts['stopped']) is bool and isinstance(facts['unresolved'], list), '收尾记录字段无效')
    repository.require(all(isinstance(facts[k], str) and facts[k].strip() for k in ('task_id', 'observed_at', 'evidence')), '收尾观察缺少具体来源')
    repository.require(not facts['stopped'] or not facts['unresolved'], '仍有未结束事项，不能确认 stopped')
    report = evidence.read(report_path)
    self_stopped = report.get('stopped_tasks', report.get('execution', {}).get('stopped_tasks'))
    repository.require(self_stopped is not False or not facts['stopped'], '子 agent 与派发者停止事实矛盾，需先更正来源')
    path = Path(dispatch_path).parent / ('closure-' + uuid.uuid4().hex + '.json')
    evidence.write(path, dict(facts, dispatch=evidence.binding(dispatch_path), report=evidence.binding(report_path)))
...
def check_close(dispatch_path, report_path, source, required=True):
    ...
    repository.require(type(record.get('stopped')) is bool and isinstance(record.get('unresolved'), list)
              and all(isinstance(record.get(k), str) and record[k].strip()
                      for k in ('task_id', 'observed_at', 'evidence')), '收尾观察字段无效')
    repository.require(record['dispatch'] == evidence.binding(dispatch_path) and record['report'] == evidence.binding(report_path), '收尾来源与交付不符')
    report = evidence.read(report_path)
    advance = report.get('status', 'COMPLETED') in ('DONE', 'READY', 'READY_TO_MERGE', 'COMPLETED') or report.get('outcome') == 'code_failure'
    if advance:
        repository.require(record['stopped'] and not record['unresolved'], '未确认任务停止，不能推进')
    return record
