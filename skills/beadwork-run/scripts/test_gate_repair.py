...
class GateRepairTests(unittest.TestCase):
    ...
    def run_gate(self, mode='fail', delivery=True, expected=1, recipe='gate-core'):
        ...
        r = subprocess.run(argv, env={**self.h.env, 'TEST_MODE': mode}, cwd=self.h.root, capture_output=True, text=True)
    ...
    def begin(self, failure, expected=0):
        r = subprocess.run([sys.executable, '-B', str(verification_fixture.ASSEMBLE), 'begin-gate-repair',
                            '--dispatch', str(self.v.dispatch), '--failure', str(failure)],
                           env=self.h.env, cwd=self.h.root, capture_output=True, text=True)
        self.assertEqual(r.returncode, expected, r.stdout + r.stderr)
    ...
    def test_each_repair_binds_new_head_and_preserves_prior_evidence(self):
        ...
        self.run_gate(mode='pass', expected=0)
        r = subprocess.run([sys.executable, '-B', str(verification_fixture.ASSEMBLE),
                            'review-prepare', '--dispatch', str(self.v.dispatch)],
                           env=self.h.env, cwd=self.h.root, capture_output=True, text=True)
    ...
    def test_resume_inherits_and_new_stage_gets_new_opportunity(self):
        ...
        self.assertEqual(self.begin(failure)['gate_repair_path'], str(old.parent / 'gate-repair.json'))
        second = self.run_gate()
        self.assertEqual(self.begin(second)["repair_number"], 2)
    ...
    def test_candidate_head_is_fixed_and_review_blocks_repair(self):
        ...
        self.begin(failure)
        self.run_gate(mode='pass', expected=0)
        r = subprocess.run([sys.executable, '-B', str(verification_fixture.ASSEMBLE), 'review-prepare', '--dispatch', str(self.v.dispatch)], env=self.h.env, cwd=self.h.root, capture_output=True, text=True)
        self.assertEqual(r.returncode, 0, r.stderr)
