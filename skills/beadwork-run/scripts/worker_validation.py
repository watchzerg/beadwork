...
def check(role, report_path, expected_path, receipt_path=None):
    ...
    if receipt_path:
        ...
        if not problems:
            wanted = {"status": status, "report_path": os.path.abspath(report_path),
                      "report_sha256": digest}
            failures.extend("receipt_" + key + "_matches"
                            for key, value in wanted.items() if receipt[key] != value)
    return {"ok": not failures, "status": status, "report_sha256": digest,
...
def main(argv):
    ...
    if len(argv) == 2 and argv[0] == "--schema" and argv[1] in ROLES:
        print(json.dumps(schema(argv[1]), ensure_ascii=False)); return
