...
def origins(d):
    ...
    result = {}
    keys = ('role', 'repository_root', 'worktree', 'branch', 'parent_id',
            'ticket_id', 'base_commit', 'attempt_id')
    for path in paths:
        origin = dispatch_contract.verification_dispatch(path)
        repository.require(all(d.get(k) == origin.get(k) for k in keys), '验证记录属于其他 ticket 或 BASE')
...
def verification_snapshot(d):
    return [item for source in origins(d).values()
...
def inspect(d, snapshot, notes, status):
    ...
    for item in snapshot:
        ...
        try:
            ...
            repository.require(start['dispatch_path'] == source['path']
                      and start['dispatch_sha256'] == source['sha256'], '验证 dispatch 身份或内容已变化')
            repository.require(start['cwd'] == d['worktree'], '验证 cwd 不符')
            text = '未完成记录；退出结果未知，需确认旧任务已结束'
            if end is not None:
                text = (f"outcome={end['outcome']}；exit_code={end['exit_code']}；"
...
def collect(current_path, prior_paths, notes, report_status, snapshots=None):
    d = dict(dispatch_contract.verification_dispatch(current_path), verification_dispatches=prior_paths)
