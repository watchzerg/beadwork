...
def clean(d):
    ...
    for wt in (d['repository_root'], d['worktree']):
        if wt == d['worktree']:
            repository.require(not repository.status(wt), '同步要求 implementation worktree 干净：' + wt)
...
def command(d, directory, argv):
    ...
    folder.mkdir()
    evidence.write(folder / 'started.json', {'argv': argv, 'head': repository.sha(d['worktree'], 'HEAD')})
    executed = process_runner.run(argv, d['worktree'], folder / 'output.log')
    result = {'argv': argv, 'started_sha256': evidence.digest(folder / 'started.json'),
              'exit_code': executed['exit_code'], 'interrupted': executed['outcome'] == 'interrupted',
              'process_group_gone': executed['process_group_gone'],
              'recorder_error': executed['error'], 'log_sha256': evidence.digest(folder / 'output.log')}
    evidence.write(folder / 'result.json', result)
    repository.require(executed['outcome'] == 'exited' and executed['process_group_gone'] and executed['exit_code'] == 0,
...
def check_result(d, path, final=False):
    ...
    for entry, argv in zip(r['commands'], expected):
        repository.require(Path(entry['path']).resolve().is_relative_to(p.parent.resolve()), '命令证据不属于本次同步')
        ...
        repository.require(record['started_sha256'] == evidence.digest(start), '同步命令开始记录已变化')
        started = evidence.read(start)
        repository.require(started['head'] == r['head'] and started['argv'] == argv and record['argv'] == argv,
                  '同步验证 HEAD 或命令不符')
...
def sync(args, final=False):
    ...
    repository.require(isinstance(parent, str) and re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]*', parent)
