...
class ReviewReuseTests(TestCase):
    def setUp(self):
        ...
        record = dict(dispatch=evidence.binding(dispatch), reviewed_base='a' * 40,
                      reviewed_head='b' * 40, axes={})
        self.sources = {}
        for axis in review_evidence.AXES:
            directory = folder / axis; directory.mkdir()
            identity = directory / 'dispatch.json'
            report = directory / 'report.json'
            evidence.write(identity, dict(axis=axis, reviewed_base='a' * 40,
                           reviewed_head='b' * 40, handoff_required=True))
            evidence.write(report, dict(axis=axis, reviewed_base='a' * 40,
                           reviewed_head='b' * 40, findings=[], notes=[]))
            receipt = directory / 'receipt.json'
            evidence.write(receipt, dict(status='COMPLETED', report_path=str(report),
                           report_sha256=evidence.digest(report)))
            closure = handoff.close(str(identity), str(report), dict(task_id='review-task', stopped=True,
                                    observed_at='2026-09-19', evidence='已观察到退出', unresolved=[]))
            self.sources[axis] = dict(report=str(report), receipt=str(receipt),
                                     closure=closure['closure_source']['path'])
    ...
    def test_same_content_reuses_checks_but_changed_closure_is_rejected(self):
        ...
        with patch.object(report_io, 'reviewer', wraps=report_io.reviewer) as checks:
            ...
            value = evidence.read(closure); value['stopped'] = False
    ...
    def test_corrected_report_and_receipt_require_new_check(self):
        ...
        value = evidence.read(old['report']); value['notes'].append('补充依据')
        evidence.write(report, value)
        receipt = report.with_name('correction-receipt.json')
        evidence.write(receipt, dict(status='COMPLETED', report_path=str(report), report_sha256=evidence.digest(report)))
        closure = handoff.close(str(report.parent / 'dispatch.json'), str(report),
                               dict(task_id='review-task', stopped=True, observed_at='2026-09-19',
                                    evidence='已观察到退出', unresolved=[]))
        self.sources['standards'] = dict(report=str(report), receipt=str(receipt), closure=closure['closure_source']['path'])
        with patch.object(report_io, 'reviewer', wraps=report_io.reviewer) as checks:
            _, _, pair, _ = review_evidence.pair_from_sources(self.round, self.sources, verified=verified)
