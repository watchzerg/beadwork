...
def prepare_attempt(d, head):
    ...
    if prior and "stage_path" in prior:
        ...
        if not changed_main and not d.get("new_attempt_reason"):
            ...
            repository.git(d["worktree"], "merge-base", "--is-ancestor", previous["stage_base"], head)
            repository.require(set(previous["required_boundary_gates"]) <= set(d["required_boundary_gates"]), "恢复不得减少 gates")
            repository.require(not repository.status(d["worktree"]) or previous["stage"] > 0, "仅修复阶段可接续 dirty 现场")
        else:
    ...
    if "attempt_id" not in d:
        evidence_root = Path(d["repository_root"]) / ".worktrees/.evidence" / d["parent_id"] / "final-review"
...
def read_stage_report(stage, report_path, receipt_path):
    for path in (report_path, receipt_path):
        repository.require(Path(path).resolve().parent == Path(stage["dispatch_path"]).parent, "阶段报告必须位于原目录")
    report_io.verifier("finalizer", "--check-report", report_path, receipt_path, "--expected", stage["dispatch_path"])
...
def models(stage, previous, facts):
    levels = dict(zip(("fixer", "standards", "spec"), workflow_policy.FINAL_STAGE_MODELS[stage]))
    if previous:
    ...
    if changes:
        ...
        for role, model in changes.items():
            repository.require(model in workflow_policy.MODEL_LEVELS and workflow_policy.MODEL_LEVELS.index(model) >= levels[role], "模型不能降档")
...
def prepare_stage(dispatch_path, facts):
    ...
    sources = (resume_stage_sources(root, previous_path, facts, head, continuation) if previous_path
    ...
    repository.require(0 <= stage < len(workflow_policy.FINAL_STAGE_MODELS), "六阶段已用尽，停止并保留现场")
...
def check_report(expected, report, *, review_checks=None):
    ...
    for source in fixes:
        ...
        repository.require(unavailable or all(v in report["verification"] for v in fr["verification"]), "遗漏 fixer 验证")
        ...
        if not new_blocking:
            if d['stage'] == 0:
                ...
                code_failure_evidence = bool(fixes and evidence.read(evidence.bound(fixes[-1]['dispatch']))['stage'] == d['stage']
                    and evidence.read(evidence.bound(fixes[-1]['report']))['outcome'] == 'code_failure')
    if report["outcome"] == "code_failure":
        repository.require(code_failure_evidence, "代码失败须有当前阶段失败验证或 blocking review/fixer 依据")
    repository.require(report["fix"]["commits"] == commits, "最终 fix commits 与原始报告不符")
    if report["head_commit"]:
        actual = repository.git(d["worktree"], "rev-list", "--reverse", d["start_head"] + ".." + report["head_commit"]).splitlines()
    ...
    if report["outcome"] == "passed":
        ...
        repository.require(reviews and report["review_rounds"][-1]["spec"]["reviewed_head"] == report["head_commit"], "最终 review 未覆盖 HEAD")
    else:
...
def accept_fixer(stage_path, report_path, receipt_path, closure=None):
    ...
    source = {k: evidence.binding(str(p)) for k, p in
    ...
    if item['fixes'] and item['fixes'][-1]['dispatch'] == item['fixer']:
        ...
        if prior['outcome'] in ('passed', 'code_failure'):
            repository.require(prior['head_commit'] == r['head_commit'] and prior['outcome'] == r['outcome'], 'fixer 终态不能改报中断或改变 HEAD')
...
def review_ready(d):
    ...
    if d['stage']:
        ...
        repository.require(fd['stage'] == d['stage'] and report['status'] == 'DONE' and report['head_commit'] == head,
...
def deliver(root_path, output):
    ...
    repository.require(r['outcome'] != 'code_failure' or r['stage'] == len(workflow_policy.FINAL_STAGE_MODELS) - 1, '代码失败需在六阶段用尽后交付 controller')
    check_report(root, r)
    repository.require(repository.sha(root['worktree'], 'HEAD') == r['head_commit'], '交付 HEAD 已变化')
    target = dispatch_contract.output_path(output, Path(root_path).parent)
    target.write_bytes(report.read_bytes())
    receipt = {'status': r['status'], 'report_path': str(target), 'report_sha256': evidence.digest(target)}
...
def inspect_delivery(dispatch_path, report_path, receipt_path):
    ...
    repository.require(not repository.git(wt, "status", "--porcelain=v1", "--untracked-files=no", "--", ".beads"), ".beads 有未提交改动")
    ...
    repository.require(evidence.digest(report_path) == result["report_sha256"], "验收期间报告发生变化")
    return d, r, result
...
def resume_stage_sources(root, previous_path, facts, head, continuation):
    ...
    for path in Path(root["attempt_path"]).glob("stage-*/dispatch.json"):
        ...
        repository.require(not any(source["path"] == str(previous_path) for source in later.get("previous_stages", [])),
    ...
    repository.require(bool(facts.get("previous_report")) == bool(facts.get("previous_receipt")), "报告和回执须成对提供")
    if facts.get("previous_report"):
        ...
        repository.require(not list(Path(previous_path).parent.glob("result-*.json")), "已有阶段结果，必须显式选择报告和回执")
    if continuation == "repair":
        repository.require(report is not None and report["outcome"] == "code_failure", "下一修复阶段需要 code_failure 证据")
        stage += 1
        base = head
    elif report:
        repository.require(report["outcome"] in ("interrupted", "blocked"), "代码失败必须进入下一阶段")
...
def publish_fixer(d, previous, fixer_done):
    ...
    if d["stage"] > 0 and not fixer_done:
        ...
        folder.mkdir()
        fd = {**d, "role": "fixer", "base_commit": d["stage_base"], "stage_dispatch": evidence.binding(d["dispatch_path"]),
...
def check_stage_history(expected, report, d, stages):
    ...
    for index, current in enumerate(stages):
        ...
        repository.require(current["previous_stages"] == report["stage_sources"][:index], "阶段历史链不完整")
    for a, b in zip(stages, stages[1:]):
        ...
        if b.get("previous_result"):
            ...
            report_io.verifier("finalizer", "--check-report", previous_report, previous_receipt, "--expected", a["dispatch_path"])
            prior = evidence.read(previous_report)
            repository.require(prior["stopped_tasks"], "历史任务未确认停止")
            repository.require(b["prior_reviews"] == prior["review_sources"] and b["prior_fixes"] == prior["fix_sources"], "阶段交接丢失报告来源")
            repository.require(prior["outcome"] == "code_failure" if b["stage"] > a["stage"] else prior["outcome"] in ("blocked", "interrupted"), "阶段交接结果不符")
            if b["stage"] > a["stage"]:
                repository.require(b["stage_base"] == prior["head_commit"], "新修复阶段 BASE 不符")
        else:
            repository.require(b["stage"] == a["stage"] and b["prior_reviews"] == a["prior_reviews"]
...
def check_review_history(d, report, reviews, *, review_checks=None):
    ...
    for index, (source, pair) in enumerate(zip(reviews, report["review_rounds"])):
        ...
        repository.require(previous_review_stage < review_stage <= d["stage"], "同阶段重复 review 或阶段顺序不符")
