...
def axis_report_schema() -> Dict[str, Any]:
    finding = object_schema({
        ...
        "blocking": {"type": "boolean"}, "title": TEXT, "evidence": TEXT})
    finding["allOf"] = [
    ...
    return {"$schema": "http://json-schema.org/draft-07/schema#", "title": "AxisReport",
            **object_schema({"reviewed_base": SHA, "reviewed_head": SHA,
                             "axis": {"enum": ["standards", "spec"]},
                             "findings": {"type": "array", "items": finding}, "notes": TEXTS})}
