...
def check(phase: str, report_path: str, expected_path: str | None = None,
          receipt_path: str | None = None) -> Dict[str, Any]:
    ...
    if receipt_path:
        ...
        if problems:
            ...
            wanted = {"status": report.get("status") if isinstance(report, dict) else None,
                      "report_path": os.path.abspath(report_path), "report_sha256": digest}
            failures.extend(fail("receipt_" + key + "_matches")
...
def main(argv: List[str]) -> None:
    ...
    if len(argv) == 2 and argv[0] == "--schema" and argv[1] in PHASES:
        print(json.dumps(schema(argv[1]), ensure_ascii=False, separators=(",", ":"))); return
