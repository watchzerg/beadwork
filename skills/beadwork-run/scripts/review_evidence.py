...
def pair_from_sources(round_path, sources, *, verified=None):
    ...
    if base == record["reviewed_head"]:
        repository.require(
            record.get("review_kind") == "existing_behavior", "空 diff 缺少已有行为审查身份"
        )
    ...
    for axis in AXES:
        ...
        repository.require(
            identity["axis"] == axis
            and identity["reviewed_base"] == base
            and identity["reviewed_head"] == record["reviewed_head"],
            "轴身份与 round 不符",
        )
        repository.require(
            identity.get("review_kind") == record.get("review_kind")
            and identity.get("acceptance_evidence") == record.get("acceptance_evidence"),
            "轴审查范围与 round 不符",
        )
        report = evidence.absolute(sources[axis]["report"])
        receipt = evidence.absolute(sources[axis]["receipt"])
        repository.require(
            report.parent == identity_path.parent and receipt.parent == identity_path.parent,
            "报告和回执必须位于该轴证据目录",
        )
        handoff.check_close(
            str(identity_path),
            str(report),
            evidence.binding(sources[axis]["closure"]) if sources[axis].get("closure") else None,
        ...
        if verified is None or key not in verified:
            checked = report_io.reviewer(
                "--check-report", report, receipt, "--expected", identity_path
            )
            repository.require(
                checked["status"] == "COMPLETED", "reviewer 未完成，保留失败证据并返回 BLOCKED"
            )
            repository.require(
                evidence.digest(report) == checked["report_sha256"], "验收期间报告发生变化"
            )
...
def collection(path, dispatch_path, *, verified=None):
    ...
    sources = {
        axis: {key: str(evidence.bound(value)) for key, value in entries.items()}
