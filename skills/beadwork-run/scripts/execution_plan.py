...
def plan(value):
    require(isinstance(value, dict) and set(value) == {'ticket_order'}, '执行计划只能包含 ticket_order')
    order = value['ticket_order']
    require(isinstance(order, list) and bool(order), 'ticket_order 必须为非空数组')
    require(all(isinstance(x, str) and re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]*', x)
                and x not in ('.', '..') for x in order), 'ticket_order 需要完整有效的 Bead ID')
    require(len(order) == len(set(order)), 'ticket_order 包含重复 ID')
    return {'ticket_order': list(order)}


def parse(body):
    require(isinstance(body, str) and body.count(START) == body.count(END) == 1,
            'parent 必须包含唯一 execution-plan 区块')
    a, b = body.index(START) + len(START), body.index(END)
    require(a < b, 'execution-plan marker 顺序错误')
    match = re.fullmatch(r'\s*```json\s*\n(.*?)\n```\s*', body[a:b], re.S)
    require(match is not None, 'execution-plan 区块需要唯一 JSON code fence')
    return plan(evidence.loads(match[1]))


def replace(body, value):
...
def validate(value, children, dependencies):
    ...
    require(isinstance(children, list) and all(isinstance(x, dict) and isinstance(x.get('id'), str)
            and isinstance(x.get('status'), str) for x in children), 'children 数据无效')
    ...
    for child, blockers in dependencies.items():
        require(isinstance(blockers, list) and all(isinstance(x, str) and x for x in blockers), 'blocking 依赖格式错误')
        for blocker in blockers:
            if blocker in positions:
                require(positions[blocker] < positions[child], f'执行顺序违反依赖：{child} 依赖 {blocker}')
...
def dependencies(children, query):
    ...
    for child in children:
        rows = query(child['id'])
        require(isinstance(rows, list) and all(isinstance(x, dict) and isinstance(x.get('id'), str)
                and x['id'] for x in rows), 'bd dep list 返回无效依赖')
...
def live(root, parent_id, value=None):
    ...
    grandchildren = bd(root, 'dep', 'list', *(x['id'] for x in children), '--direction=up', '--type=parent-child')
    require(not grandchildren, '图未平铺')
    deps = dependencies(children, lambda child: bd(root, 'dep', 'list', child, '--direction=down', '--type=blocks'))
...
def folder(root, parent_id):
    require(isinstance(parent_id, str) and re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]*', parent_id)
            and parent_id not in ('.', '..'), 'parent ID 无效')
    return Path(repository.primary(root)) / '.worktrees' / '.evidence' / parent_id / 'execution-plan'
...
def selected(root, parent_id, required=True):
    ...
    while True:
        ...
        require(record['parent_id'] == parent_id and record['repository_root'] == str(directory.parents[3]), '计划归属错误')
        plan(record['plan'])
        require(record['previous'] == previous, '计划选择来源链断裂')
        sources = [evidence.read(evidence.bound(source)) for source in record['sources']]
        require(any(isinstance(source, dict) and
                    (source.get('execution_plan') == record['plan'] or
                     source.get('ticket_order') == record['plan']['ticket_order']) for source in sources),
                '计划与批准来源不符')
...
def check_selected(root, parent_id, value, children, expected=None, required=True):
    ...
    if binding is None:
        require(all(x['status'] == 'open' and not x.get('assignee') for x in children),
...
def adopt(root, parent_id, value, children, sources, reason, previous=None):
    ...
    if current is not None:
        ...
        if old['plan'] == plan(value):
            if previous is not None and previous != current:
                require(old['previous'] == previous and old['sources'] == sources and old['reason'] == reason,
                        '重复接纳来源不符')
            check_selected(root, parent_id, value, children)
            return current
        require(previous == current, '调整需要绑定当前选定计划')
        sync_dir = directory.parent / 'main-sync'
        require(not any(not (path.parent / 'ready.json').exists() for path in sync_dir.glob('*/intent.json')),
        ...
        protected = {x for x in old_order if states[x] in ('closed', 'in_progress') or (directory / progress_name(x, 'started')).exists()}
        require(all(old_order.index(x) == new_order.index(x) for x in protected), '不得移动已关闭或活动票')
        # 当前 writer 未结束时不接纳重排；恢复原票无需重排。
        require(not any(x['status'] == 'in_progress' for x in children), '活动票尚未结束，不能接纳重排')
    else:
    ...
    target = directory / ('initial.json' if current is None else 'after-' + current['sha256'] + '.json')
    record = dict(repository_root=str(directory.parents[3]), parent_id=parent_id, plan=plan(value),
                  closed=[x['id'] for x in children if x['status'] == 'closed'], previous=current,
