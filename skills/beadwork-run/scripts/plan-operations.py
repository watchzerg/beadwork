...
def prepare(source, output):
    ...
    record = dict(repository_root=root, parent_id=data['parent_id'], plan=value,
                  before=body, after=after, parent_status=parent['status'])
...
def publish(intent_path):
    ...
    plans.require(data['after'] == plans.replace(data['before'], data['plan']), '发布 intent 正文不匹配')
    parent, _, _, _ = plans.live(root, parent_id, data['plan'])
    plans.require(parent.get('description', '') in (data['before'], data['after']), 'parent 正文已变化，请重新 prepare')
    ...
    plans.require(parent.get('description') == data['after'] and parent['status'] == data['parent_status']
                  and value == data['plan'], '发布读回不一致，保留现场')
...
def adopt(source):
    data = evidence.read(source)
    plans.require(set(data) == {'repository_root', 'parent_id', 'ticket_order', 'previous', 'reason'}, '接纳输入字段无效')
    root = repository.primary(data['repository_root'])
    ...
    plans.require(value == plans.plan({'ticket_order': data['ticket_order']}), 'parent 与批准顺序不同')
    return plans.adopt(root, data['parent_id'], value, children, [evidence.binding(source)],
                       data['reason'], data['previous'])


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    subs = parser.add_subparsers(dest='command', required=True)
    p = subs.add_parser('prepare'); p.add_argument('--input', required=True); p.add_argument('--output', required=True)
    p = subs.add_parser('publish'); p.add_argument('--intent', required=True)
    p = subs.add_parser('adopt'); p.add_argument('--input', required=True)
    p = subs.add_parser('inspect'); p.add_argument('--repository-root', required=True); p.add_argument('--parent', required=True)
    ...
    if args.command == 'prepare':
        ...
        result = {'plan': value, 'selected': plans.selected(args.repository_root, args.parent, False),
