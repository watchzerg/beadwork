...
def run(folder, name, argv, cwd, context=None):
    ...
    result.update(started=evidence.binding(directory / 'started.json'),
...
def read(source):
    ...
    repository.require(Path(value['started']['path']).parent == path.parent
                       and Path(value['log']['path']).parent == path.parent, '命令证据目录不符')
