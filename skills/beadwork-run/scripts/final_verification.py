...
def records(d, sources, allowed, notes, successful=False):
    ...
    for item in sources:
        ...
        repository.require(
            evidence.binding(str(source)) in allowed and path.parent.parent == source.parent,
            "验证不属于已绑定阶段或 fixer",
        )
        ...
        if successful and not valid:
            repository.require(
                str(path.parent) in notes and str(notes[str(path.parent)]).strip(),
                "未知或无效验证需要实际收尾说明",
            )
        for gate in gates:
            rows.append(
                (
                    ...
                    valid,
                    {
                        ...
                        "head_commit": start["before"]["head"],
                        "passed": bool(valid and end["exit_code"] == 0),
                        "result": (
                            "outcome=" + end["outcome"] + "; exit_code=" + str(end["exit_code"])
                        )
                        if end
                        else "结果未知",
...
def check_snapshot(d, report):
    ...
    own = [
        s
        for s in report["verification_sources"]
...
def check(d, report, live=False):
    ...
    repository.require(
        not issues
        or (report["status"] == "BLOCKED" and report.get("outcome") in ("blocked", "interrupted")),
    ...
    if report.get("outcome") == "code_failure" and d["role"] == "fixer":
        repository.require(
            gate_repair.used_repairs(gate_repair.root(d)) == 3, "fixer 代码失败须用尽三次修复"
        )
        repository.require(
            any(
...
def populate(d, report, inherited=()):
    ...
    for binding in allowed(d, report):
        ...
        selected = [
            s
            for s in inherited
            if verification_records.directory(s).parent == Path(origin["dispatch_path"]).parent
        ]
        captured = selected
        try:
            ...
            repository.require(
                ...
                and report.get("outcome") in ("blocked", "interrupted"),
                "验证来源损坏；保留现场并以 blocked/interrupted 交付：" + str(error),
            )
            report["verification_issues"].append(
                {"dispatch": binding, "reason": str(error), "verification_sources": captured}
            )
            selected = []
    ...
    report["verification"] = [
        row[-1] for row in records(d, sources, allowed(d, report), report["verification_notes"])
    ]
