...
class PhaseValidatorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.expected = {"parent_id": "demo-1", "expected_children": ["demo-2", "demo-3"], "reviewed_main": SHA_A, "start_head": SHA_B, "required_boundary_gates": ["gate-browser"]}

    def dispatch(self, phase: str) -> dict:
        common = {"repository_root": "/repo", "parent_id": "demo-1", "rules_paths": [], "skill_dir": "/skill", "report_path": "/evidence/report.json", "dispatch_path": "/evidence/dispatch.json"}
        if phase == "preflight":
            return {**common, "expected_branch": "implement/demo-1", "expected_worktree": "/repo/.worktrees/demo-1"}
        return {**common, "worktree": "/repo/.worktrees/demo-1", "branch": "implement/demo-1", "expected_children": self.expected["expected_children"], "linked_spec": "spec", "reviewed_main": SHA_A, "start_head": SHA_B, "ticket_evidence": [], "required_boundary_gates": ["gate-browser"], "prior_finalization": None}
    ...
    def pair(self, base: str = SHA_A, head: str = SHA_B, blocking: bool = False) -> dict:
        finding = [{"axis": "spec", "kind": "defect", "blocking": True, "title": "缺陷", "evidence": "证据"}] if blocking else []
    ...
    def invoke(self, phase: str, report: dict, *, expected: bool | dict = True, receipt: dict | None = None) -> dict:
        report_path = self.root / (phase + ".json")
        raw = json.dumps(report, ensure_ascii=False).encode(); report_path.write_bytes(raw)
        args = ["--check-report", phase, str(report_path)]
        if receipt is not None:
            receipt_path = self.root / "receipt.json"; receipt_path.write_text(json.dumps(receipt)); args.append(str(receipt_path))
        if expected:
            dispatch = self.root / "dispatch.json"; dispatch.write_text(json.dumps(expected if isinstance(expected, dict) else self.dispatch(phase))); args.extend(["--expected", str(dispatch)])
        result = subprocess.run([sys.executable, str(VERIFIER), *args], capture_output=True, text=True)
    ...
    def test_preflight_rejects_missing_check_gate_and_changed_children(self) -> None:
        changed = self.preflight(); changed["checks"] = []
        self.rejected("preflight", changed, "all_preflight_checks_passed")
        changed = self.preflight(); changed["boundary_gates"] = []
        self.rejected("preflight", changed, "boundary_gates_are_ticket_union")
        changed = self.preflight(); changed["expected_children"] = ["demo-2"]
    ...
    def test_finalizer_rejects_wrong_sha_omitted_gate_repair_and_live_writer(self) -> None:
        changed = self.finalizer(); changed["review_rounds"] = [self.pair(base="c" * 40)]
        self.rejected("finalizer", changed, "review_sha_binding")
        changed = self.finalizer(); changed["verification"] = []
        self.rejected("finalizer", changed, "required_gates_covered")
        changed = self.finalizer(); changed["fix"] = {"used": False, "commits": ["c" * 40], "dispositions": []}
        self.rejected("finalizer", changed, "repair_limit")
        changed = self.finalizer(); changed["stopped_tasks"] = False
    ...
    def test_replacement_cannot_reset_used_repair_or_review_rounds(self) -> None:
        ...
        expected["prior_finalization"] = {"fix_used": True, "review_rounds_used": 2, "report_path": "/evidence/prior.json"}
        report = self.finalizer()
    ...
    def test_dispatch_without_snapshot_and_valid_prior(self) -> None:
        ...
        for prior in ({}, {"fix_used": "true", "review_rounds_used": 1}, {"fix_used": True, "review_rounds_used": True, "report_path": "/evidence/prior.json"}):
            expected = self.dispatch("finalizer")
    ...
    def test_receipt_binds_path_status_and_hash(self) -> None:
        report = self.preflight(); path = self.root / "preflight.json"; raw = json.dumps(report, ensure_ascii=False).encode()
        path.write_bytes(raw)
        receipt = {"status": "READY", "report_path": str(path.absolute()), "report_sha256": hashlib.sha256(raw).hexdigest()}
