...
class ExecutionPlanTests(unittest.TestCase):
    def setUp(self):
        ...
        self.state = {'parent': {'id': 'p-1', 'status': 'open', 'description': plans.replace('保留正文', self.value)},
                      'children': [{'id': x, 'status': 'open', 'labels': ['ready-for-agent']} for x in self.order],
                      'deps': {}, 'ready': list(reversed(self.order)), 'writes': []}
        self.state_path = self.root / 'state.json'; self.save()
        binary = self.root / 'bd'
...
''')
        binary.chmod(0o755)
        self.env.update(PATH=str(self.root) + os.pathsep + self.env['PATH'], PLAN_STATE=str(self.state_path))
        self.patch = patch.dict(os.environ, self.env); self.patch.start(); self.addCleanup(self.patch.stop)
        source = self.root / 'approved.json'; evidence.write(source, self.value)
        self.binding = plans.adopt(self.root, 'p-1', self.value, self.state['children'], [evidence.binding(source)], '测试批准')
    ...
    def intent(self, name, ticket=None, kind='claim', **extra):
        source = self.root / (name + '-input.json'); target = self.root / (name + '.json')
        evidence.write(source, dict(repository_root=str(self.root), parent_id='p-1', issue_id=ticket or self.order[0],
                                    kind=kind, expected_assignee='fixture', **extra))
    ...
    def test_next_blocked_never_skips_to_ready_later(self):
        self.state['ready'] = self.order[1:]; self.save()
        self.assertEqual(self.next()['reason'], 'next_ticket_not_ready')

    def test_missing_plan_never_falls_back(self):
        self.state['parent']['description'] = '没有计划'; self.save()
        self.assertEqual(self.next()['next'], 'blocked')

    def test_invalid_dependency_and_scope(self):
        self.state['deps'][self.order[0]] = [self.order[1]]; self.save()
        self.assertIn('违反依赖', self.next()['detail'])
        self.state['deps'] = {}; self.state['children'].pop(); self.save()
        self.assertEqual(self.next()['next'], 'blocked')

    def test_formatting_and_unrelated_parent_changes_are_allowed(self):
        self.state['parent']['description'] = '新说明\n' + plans.START + '\n```json\n' + json.dumps(self.value) + '\n```\n' + plans.END
        self.save(); self.assertEqual(self.next()['next'], 'claim')

    def test_reorder_requires_explicit_adoption_and_keeps_history(self):
        new = {'ticket_order': list(reversed(self.order))}
        self.state['parent']['description'] = plans.replace('正文', new); self.save()
        self.assertEqual(self.next()['next'], 'blocked')
        source = self.root / 'reorder.json'
        evidence.write(source, dict(repository_root=str(self.root), parent_id='p-1', **new,
                                    previous=self.binding, reason='批准剩余票重排'))
    ...
    def test_claim_guard_and_stale_intent(self):
        ...
        tracker.execute(good); tracker.execute(good)
    ...
    def test_reopened_completed_ticket_blocks(self):
        claim = self.intent('start'); tracker.execute(claim)
        report = self.root / 'accepted.json'; evidence.write(report, {'status': 'DONE'})
        close = self.intent('close', kind='close', reason='已验收', prerequisite=evidence.binding(report))
        tracker.execute(close)
        self.assertEqual(self.next()['ticket_id'], self.order[1])
        self.state = json.loads(self.state_path.read_text()); self.state['children'][0]['status'] = 'open'; self.save()
    ...
    def test_publish_preserves_body_and_detects_concurrent_edit(self):
        ...
        evidence.write(source, dict(repository_root=str(self.root), parent_id='p-1', ticket_order=list(reversed(self.order))))
        ...
        state['parent']['description'] += '\n人工编辑'; self.state = state; self.save()
        self.assertIn('正文已变化', self.call('plan-operations.py', 'publish', '--intent', intent, ok=False)['error'])

    def test_active_and_closed_tickets_cannot_be_moved(self):
        self.state['children'][0]['status'] = 'closed'; self.save()
        new = {'ticket_order': list(reversed(self.order))}
        self.state['parent']['description'] = plans.replace('', new); self.save()
        source = self.root / 'reorder-protected.json'
        evidence.write(source, dict(repository_root=str(self.root), parent_id='p-1', **new,
                                    previous=self.binding, reason='错误移动已关闭票'))
        self.assertIn('不得移动', self.call('plan-operations.py', 'adopt', '--input', source, ok=False)['error'])
        self.state['children'][0]['status'] = 'open'; self.state['children'][1]['status'] = 'in_progress'
        self.state['parent']['description'] = plans.replace('', self.value); self.save()
    ...
    def test_plan_adoption_invalidates_prepared_claim(self):
        ...
        evidence.write(source, dict(repository_root=str(self.root), parent_id='p-1', **new,
                                    previous=self.binding, reason='批准重排未开始部分'))
    ...
    def test_external_blocker_and_completed_batch(self):
        ...
        self.state['ready'] = self.order[1:]; self.save()
        self.assertEqual(self.next()['reason'], 'next_ticket_not_ready')
        for child in self.state['children']: child['status'] = 'closed'
        self.save(); self.assertEqual(self.next(), {'next': 'done'})

    def test_pending_sync_prevents_plan_adoption(self):
        pending = self.root / '.worktrees/.evidence/p-1/main-sync/pending'
        pending.mkdir(parents=True); evidence.write(pending / 'intent.json', {'pending': True})
        new = {'ticket_order': list(reversed(self.order))}
        self.state['parent']['description'] = plans.replace('', new); self.save()
        source = self.root / 'reorder-pending.json'
        evidence.write(source, dict(repository_root=str(self.root), parent_id='p-1', **new,
                                    previous=self.binding, reason='批准重排'))
        self.assertIn('未完成 main-sync', self.call('plan-operations.py', 'adopt', '--input', source, ok=False)['error'])
    ...
    def test_no_adoption_or_migration_without_initial_preflight(self):
        ...
        evidence.write(source, dict(repository_root=str(self.root), parent_id='p-1', **self.value,
                                    previous=None, reason='不能作为初始化入口'))
        self.assertIn('previous', self.call('plan-operations.py', 'adopt', '--input', source, ok=False)['error'])
        with self.assertRaisesRegex(ValueError, '未开工批次'):
            plans.adopt(self.root, 'another-parent', {'ticket_order': ['another-1']},
                [{'id': 'another-1', 'status': 'in_progress'}], [evidence.binding(source)], '不能导入活动票')

    def test_strict_parser_and_no_version(self):
        for body in ('', plans.replace('', self.value) * 2,
                     plans.START + '\n```json\n{"ticket_order":["x"],"version":1}\n```\n' + plans.END,
                     plans.START + '\n```json\n{"ticket_order":["x"],"ticket_order":["y"]}\n```\n' + plans.END):
            with self.subTest(body=body), self.assertRaises(ValueError): plans.parse(body)
        with self.assertRaises(ValueError): plans.plan({'ticket_order': ['x', 'x']})
