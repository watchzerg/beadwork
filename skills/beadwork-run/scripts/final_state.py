...
def state(d):
    ...
    for number, path in enumerate(files, 1):
        ...
        repository.require(
            path.name == f"checkpoint-{number:06d}.json" and item["previous"] == previous,
            "最终检查点链不连续",
        )
        repository.require(
            item["attempt_id"] == d["attempt_id"] and item["state_sha256"] == digest(item["state"]),
            "最终检查点身份或状态已变化",
        )
        value = item["state"]
...
def save(d, value):
    ...
    evidence.write(
        Path(d["attempt_path"]) / f"checkpoint-{number + 1:06d}.json",
        {
...
def selected(d, current=True):
    ...
    repository.require(
        item["dispatch"] == evidence.binding(d["dispatch_path"]), "阶段 dispatch 与检查点不符"
    )
    if current:
...
def reserve_review(d, resume=False):
    ...
    if item["round_path"]:
        repository.require(
            resume and item["round"] is None, "本阶段已有 review；复用原 round 或更正报告"
        )
...
def select_report(d, report, receipt):
    ...
    item["report"] = {
        ...
        for k, p in (("dispatch", d["dispatch_path"]), ("report", report), ("receipt", receipt))
    }
...
def result(d):
    ...
    if (
        item["fixes"]
        and evidence.read(evidence.bound(item["fixes"][-1]["dispatch"]))["stage"] == d["stage"]
    ):
