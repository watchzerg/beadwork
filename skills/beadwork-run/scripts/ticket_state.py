...
def source(dispatch, report, receipt):
    return {
        k: evidence.binding(str(v))
        for k, v in (("dispatch", dispatch), ("report", report), ("receipt", receipt))
    }


def resolve_source(item):
    repository.require(
        set(item) == {"dispatch", "report", "receipt"}, "来源必须含 dispatch/report/receipt"
    )
    paths = {k: evidence.bound(v) for k, v in item.items()}
    repository.require(
        all(paths[k].parent == paths["dispatch"].parent for k in ("report", "receipt")),
        "来源目录不符",
    )
    ...
    repository.require(
        ...
        == {
            ...
            "report_sha256": evidence.digest(paths["report"]),
        },
        "来源回执不符",
    )
...
def checkpoints(d):
    ...
    for i, path in enumerate(files, 1):
        ...
        repository.require(
            entry["previous"] == previous and entry["root"] == root_binding,
            "检查点链路或 root 已变化",
        )
        repository.require(
            entry["state_sha256"] == state_digest(entry["state"]), "检查点状态已变化"
        )
        state = entry["state"]
...
def checkpoint(d, state):
    ...
    evidence.write(
        path,
        {
            "root": evidence.binding(r["dispatch_path"]),
            "previous": previous,
            "state": state,
...
def reserve_review(d, resume=False):
    ...
    repository.require(
        ...
        "只能为当前 stage 准备 review",
    )
    if state["review_round_path"]:
        repository.require(
            resume and state["review_round"] is None, "本阶段已有 review；复用原 round 或更正报告"
        )
...
def bind_review_round(d, path):
    ...
    repository.require(
        state["stage_dispatch"] == evidence.binding(d["dispatch_path"])
        and state["review_round_path"] == str(path),
        "review round 未获当前 stage 选择",
    )
...
def select_review(d, collection_path):
    ...
    repository.require(
        record["dispatch"] == state["stage_dispatch"]
        and round_path.parent.parent == Path(d["dispatch_path"]).parent,
        "review round 不属于当前 stage",
    )
    repository.require(
        state["review_round"] == collection["round"], "只能选择检查点绑定的 review round"
    )
...
def check_selected_review(d, sources, *, state=None):
    ...
    if state["stage_dispatch"] == evidence.binding(d["dispatch_path"]):
        expected = d["prior_reviews"] + (
            [state["selected_review"]] if state["selected_review"] else []
        )
        repository.require(
            sources == expected, "阶段报告必须保留检查点选中的完整 review；更正后需重新组装"
        )
