...
- [x] **6.1 最终扫描。** 核对唯一入口、第三方 import、开发/运行目录、工具版本声明、marker 完整性、重复门禁执行与失效文件。清除迁移遗留代码，不删除有独立价值的业务回归。
- [x] **6.2 整理长期维护说明。** README 保留安装与常用命令；AGENTS.md 保留约束与验证选择；ARCHITECTURE 描述最终模块职责。版本数字以配置/锁文件为准，除运行下限外避免在多处重复补丁版本。本文保留阶段记录，不替代长期文档。
- [x] **6.3 固定验证选择表。** 将下节规则落实到维护文档，区分本地 fixture workflow、真实 Beads 服务与真实 Codex 派发的证据范围。记录用户 validator 是未锁定的宿主依赖，这是已接受的设计。
- [x] **6.4 最终完整验证。** locked install 后运行 gate-full；确认完整测试包含分发，收集数量与主层数量一致。验证必要前置条件缺失时明确失败，避免重复运行已经在本次门禁成功的同一批检查。
- [x] **6.5 自行 review 并修正。** 对照第 1 节逐项检查；检查 CLI 组是否遗漏能力、业务语义是否意外变化、类型豁免是否过宽、测试是否只重复实现、有没有引入未请求的兼容/发布机制。修正后仅重跑受影响检查，必要时完整门禁。
- [x] **6.6 完成交接。** 填写最终结果、验证限制与可能后续事项，确认各阶段任务真实完成。没有真实 agent/Beads 验收时明确说明，不因缺少范围外验收将本计划无限延长。
...
| 6 最终验收 | 已完成（2026-09-19） | 最终扫描确认 skill 只保留 `beadwork.py` 公开 CLI，10 个顶层命令组覆盖迁移清单中的 12 个旧入口能力；运行源码只依赖标准库/自带模块，marker、版本声明、开发/分发目录和 just 门禁边界一致。修正一处历史设计文档中仍可被误用的旧 CLI 示例；README、AGENTS.md 与 ARCHITECTURE 的长期职责分工核对通过，AGENTS.md 新增按修改边界选择验证的表格，明确本地 fixture、真实 Beads 与真实 Codex 派发的证据范围，并记录用户 validator 是已接受但未锁定的宿主依赖。`just install` 在 uv 0.12.10、Python 3.14.7 下 locked 收敛；单次 `just gate-full` 通过文档/结构、Ruff、ty、338/338 pytest（112.39 秒）和真实 validator。只收集统计为 unit 18、integration 141、workflow 179，主层合计 338；distribution 8 项属于 integration 且在完整收集中只出现一次。最终自审未发现遗漏 CLI、业务语义漂移、过宽类型豁免、镜像测试或未请求的兼容/发布机制。代码提交：待本次提交后补记。 | 本次没有运行真实消费项目 ticket graph、真实 Beads workspace/tracker 写入或真实 Codex 嵌套派发；脚本、临时 Git/worktree、Beads fixtures 和当前 macOS 分发验收不能替代这些范围外证据，也不代表 Linux/Windows 矩阵。六阶段计划至此结束；后续只按具体维护需求使用 AGENTS.md 的验证选择，不自动开启新的工程化阶段。未 push。 |
