...
| 6 最终验收 | 已完成（2026-09-19） | 最终扫描确认 skill 只保留 `beadwork.py` 公开 CLI，10 个顶层命令组覆盖迁移清单中的 12 个旧入口能力；运行源码只依赖标准库/自带模块，marker、版本声明、开发/分发目录和 just 门禁边界一致。修正一处历史设计文档中仍可被误用的旧 CLI 示例；README、AGENTS.md 与 ARCHITECTURE 的长期职责分工核对通过，AGENTS.md 新增按修改边界选择验证的表格，明确本地 fixture、真实 Beads 与真实 Codex 派发的证据范围，并记录用户 validator 是已接受但未锁定的宿主依赖。`just install` 在 uv 0.12.10、Python 3.14.7 下 locked 收敛；单次 `just gate-full` 通过文档/结构、Ruff、ty、338/338 pytest（112.39 秒）和真实 validator。只收集统计为 unit 18、integration 141、workflow 179，主层合计 338；distribution 8 项属于 integration 且在完整收集中只出现一次。最终自审未发现遗漏 CLI、业务语义漂移、过宽类型豁免、镜像测试或未请求的兼容/发布机制。代码提交：`facde57`。 | 本次没有运行真实消费项目 ticket graph、真实 Beads workspace/tracker 写入或真实 Codex 嵌套派发；脚本、临时 Git/worktree、Beads fixtures 和当前 macOS 分发验收不能替代这些范围外证据，也不代表 Linux/Windows 矩阵。六阶段计划至此结束；后续只按具体维护需求使用 AGENTS.md 的验证选择，不自动开启新的工程化阶段。未 push。 |
