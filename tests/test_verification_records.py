...
class VerificationRecordTests(TestCase):
    ...
    def run_record(self, number):
        ...
        evidence.write(
            ...
            dict(
                dispatch_path=str(self.dispatch),
                dispatch_sha256=evidence.digest(self.dispatch),
                cwd=str(self.root),
                started_ns=number,
                argv=["just", "--one", "--", "gate-core"],
    ...
    def test_collect_reads_each_record_once_without_directory_scan(self):
        ...
        with (
            patch.object(Path, "glob", side_effect=AssertionError("固定快照不应重新扫描")),
            patch.object(verification_records, "read", wraps=verification_records.read) as reads,
        ):
    ...
    def test_missing_and_truncated_started_preserve_other_records(self):
        ...
        self.assertEqual(
            [item["source"]["directory"] for item in issues], [str(missing), str(truncated)]
        )
    ...
    def test_historical_snapshot_does_not_discover_new_runs_or_terminal_results(self):
        ...
        self.assertEqual(
            ticket_verification.collect_verification(self.d, snapshot, {}, "BLOCKED"), before
        )
